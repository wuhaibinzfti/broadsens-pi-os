...
1. build libnl
  1) cd libnl
  2) tar -xvf libnl-3.4.0.tar.gz
  3) cd ..
  4) make libnl-config
  5) make libnl
  6) make libnl-install
  
2. build openssl
  1) cd openssl
  2) tar -xvf openssl-1.0.2o.tar.gz
  3) cd ..
  4) make openssl-config
  5) vi openssl/openssl-1.0.2o/Makefile [search '-m64' options & delet]
     modify info:
	 ...
	 quit & save
  6) make openssl
  7) make openssl-install
  
 3. build wpa_supplicant tool
  1) cd wpa_supplicant
  2) tar -xvf wpa_supplicant-2.6.tar.gz
  3) cd ..
  4) make wpa_supplicant-config
  5) export PKG_CONFIG_PATH=$PWD/build/libnl/lib/pkgconfig/:$PKG_CONFIG_PATH
  6) make wpa_supplicant
